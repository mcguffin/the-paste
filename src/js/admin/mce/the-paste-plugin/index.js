...
(function($){
	the_pastePluginCallback = function( editor ) {
		...
		editor
			...
			.on( 'PastePreProcess', function(e){
				if ( e.content.indexOf('<img') === 0 ) {
					currentPasted++;
					e.content = '<img id="thepasted-'+currentPasted.toString()+'"' + e.content.substr(4);
				}
			...
			.on( 'PastePostProcess', function(e){
				...
				if ( $firstChild.is('img') ) {
					...
					el.onload = function(e) {
						var should_upload = (this.naturalWidth * this.naturalHeight) > thepaste.options.editor.force_upload_size
						if ( thepaste.options.editor.can_upload && (thepaste.options.editor.auto_upload || should_upload ) ) {
							$uploadBox = thepaste.uploadImage( $firstChild.get(0), editor );
							$firstChild.remove();
							$(e.node).append( $uploadBox );
						} else if ( ! thepaste.options.editor.can_upload && should_upload ) {
							editor.$('#thepasted-'+currentPasted.toString()).remove();
						}
					}
				}
			})
